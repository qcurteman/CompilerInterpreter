...
This program accepts a sentence in the language HLL5 and translates it into a semantically
equivalent sentence in the language ILL5.

Grammer of HLL5:
<HLL5-sentence>   -> <varDeclSection> <mainProgSection> '.'
<varDeclSection>  -> 'DECLARE' <varIdent> { ',' <varIdent> } ';'
<varIdent>        -> <letter> { <letter> | <digit> }
<mainProgSection> -> 'BEGIN' <statSequence> 'END'
<statSequence>    -> <statement> { ';' <statement> }
<statement>       -> <i-assignStat> | <writeStat> | <ifStat>								//New "<ifStat>"
<ifStat>		  -> 'IF' <condition> 'THEN' <statSequence> [ 'ELSE' <statSequence> ] 'END' //New
<condition>		  -> <i-expression> <relOp> <i-expression>									//New
<relOp>			  -> '=' | '#' | '<' | '<=' | '>' | '>='									//New
...
(1) All variable identifiers in an HLL5 sentence must be declared first before they are used in the
<mainProgSection>;
...
#define MaxInt   32767
#define tableMax 100
#define wLeng    8			//length of longest varIdent or HLL5 reserved word
#define resWords 8			//Number of reserved words in HLL5
...
private:
	...
	enum symbols{
		...
		eqlSym, neqSym, lessSym, gtrSym, geqSym, leqSym
	};
	...
	void CGjumpOnFalse(int arg)		  { gen(jmz, -1); }
	void CGJump(int arg)			  { gen(jmp, -1); }
...
{
	cout << endl << " ===  HLL5 Compiler  === " << endl << endl
		<< "This program accepts as input an HLL5 sentence of arthmetic expressions" << endl
...
void compiler::initialize(void)
{
	...
	//list of HLL5 reserved words, listed in ascending order
	strcpy_s(resWordList[1], "BEGIN");
	strcpy_s(resWordList[2], "DECLARE");
	strcpy_s(resWordList[3], "ELSE");
	strcpy_s(resWordList[4], "END");
	strcpy_s(resWordList[5], "ENDL");
	strcpy_s(resWordList[6], "IF");
	strcpy_s(resWordList[7], "THEN");
	strcpy_s(resWordList[8], "WRITE");

	//grammar-symbol list of those reserved words
	resSymList[1] = beginSym;
	resSymList[2] = declareSym;
	resSymList[3] = elseSym;
	resSymList[4] = endSym;
	resSymList[5] = endlSym;
	resSymList[6] = ifSym;
	resSymList[7] = thenSym;
	resSymList[8] = writeSym;
...
{
	// <HLL5-sentence> -> <varDeclaration> <vainProgSection> '.'
...
{
	// <statement> -> <i-assignStat> | <writeStat> | <ifStat>
	switch (sym)
	{
...
void compiler::statementSequence(void)
{
	...
	{
		getSym();
		if ((sym == varIdentSym) || (sym == writeSym) || (sym == endlSym) || (sym == ifSym))
			statement();
		else
			error(13);
...
void compiler::gen(opCodes op, int arg)
{
	...
	if (op == ldi || op == inc || op == lda || op == jmp || op == jmz)
