...
#endif
/*	PROGRAM HLL4_Compiler

This program accepts a sentence in the language HLL4 and translates it into a semantically
equivalent sentence in the language ILL4.

Grammer of HLL4:
<HLL4-sentence>   -> <varDeclSection> <mainProgSection> '.'
...
(1) All variable identifiers in an HLL4 sentence must be declared first before they are used in the
    <mainProgSection>;
...
Grammer of ILL4:
<ILL4-sentence>  -> <p-instruction> { <p-instruction> } 'HLT'
...
#define MaxInt   32767
#define tableMax 100
#define wLeng    8			//length of longest varIdent or HLL4 reserved word
#define resWords 5			//Number of reserved words in HLL4
...
private:
	...
	enum symbols{
		...
		beginSym, endSym, writeSym, commaSym, endlSym, stringSym
	};
...
{
	cout << endl << " ===  HLL4 Compiler  === " << endl << endl
		<< "This program accepts as input an HLL4 sentence of arthmetic expressions" << endl
		<< "given in infix notation. It translates it into an equivalent ILL4 " << endl
...
void compiler::initialize(void)
{
	...
	//list of HLL4 reserved words, listed in ascending order
	strcpy_s(resWordList[1], "BEGIN");
	strcpy_s(resWordList[2], "DECLARE");
	strcpy_s(resWordList[3], "END");
	strcpy_s(resWordList[4], "ENDL");
	strcpy_s(resWordList[5], "WRITE");

	//grammar-symbol list of those reserved words
	resSymList[1] = beginSym;
	resSymList[2] = declareSym;
	resSymList[3] = endSym;
	resSymList[4] = endlSym;
	resSymList[5] = writeSym;
	

	// mnemonics of ILL4 p-code
...
{
	// <HLL4-sentence> -> <varDeclaration> <vainProgSection> '.'
...
void compiler::statementSequence(void)
{
	...
	{
		getSym();
		if ((sym == varIdentSym) || (sym == writeSym) || (sym == endlSym))
			statement();
		else
			error(13);
