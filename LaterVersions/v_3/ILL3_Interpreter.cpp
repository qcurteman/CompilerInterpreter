...
#endif
/* PROGRAM ILL3_Interpreter

This program is an interpreter (evaluator) of an ILL3 sentence which consists of a sequence of
P-instructions. An ILL3 sentence is generated by the PROGRAM HLL3_Compiler. The result of a
...
private:
	enum opCodes { add, sub, mul, dvd, ldi, lda, ldv, prn, prs, prc, nln, hlt, sto, inc, nul };
...
{
	char str[20];
	cout << endl << " === ILL3 Interpreter === " << endl << endl;
	cout << "This interpreter accepts an ILL3 sentence and interpretes its statements." << endl
...
void interpreter::loadCode(void)
{
	...
	while (!codeFile.eof())
	{
		...
		if (!codeFile.eof())
		{
			...
			if (memory.pCode[nextCode].op == ldi || memory.pCode[nextCode].op == inc || memory.pCode[nextCode].op == lda)
